...
7) 責務分離（操作ハンドル vs 解結果）
   ...
   - UI 側でハンドル用ボーンを別管理する仕組みを検討する（未実装）。
...
10) PMX の IKRotationLimit の活用
   ...
   - limitFunc を IkLink/IkBone の RotationLimit を直接返す形に変更後、足IK などで過剰な制限がないか手動確認する（未実施）。
...
- 座標系統一後、ライン描画等で軸の向きが期待通りか手動確認する。
...
- 追加材質モーフ項目のパフォーマンス検証（変更後の描画負荷を手動チェックする）
- 大量の UV モーフ適用時に頂点バッファ更新コストが高い。バッチ更新などの最適化手法を検討する。
