...
- [ ] タイムライングリッドの補間表示とキー編集機能
  - [ ] MotionEditor と MotionData の連携を拡張し、各キーフレームのボーン変換を保存する
- [x] 再生/一時停止ボタンを画像アイコンへ変更
...
- [ ] タイムライン表示のパフォーマンス最適化とスクロール位置固定処理の検証
   - [x] SKCanvasView でグリッドやキーフレームをまとめて描画し、Grid 要素の大量生成を回避
   - [ ] キーフレーム追加・削除時は対象セルのみ再描画するロジックへ変更
- [x] タイムラインのスクロール同期処理を追加テスト
