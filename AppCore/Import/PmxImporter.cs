    ...
    private void GenerateSubMeshes(ModelData data, dynamic[] verts, Surface[] faces, dynamic[] mats, string[] texList, string? textureDir)
    {
        ...
        foreach (var mat in mats)
        {
            ...
            if (!string.IsNullOrEmpty(dir))
            {
                ...
                if (mat.SphereTexture >= 0 && mat.SphereTexture < texList.Length)
                {
                    TryLoadTexture(smd, texList[mat.SphereTexture], dir, TextureType.Sphere);
                }
                if (mat.ToonTexture >= 0 && mat.ToonTexture < texList.Length)
                {
                    TryLoadTexture(smd, texList[mat.ToonTexture], dir, TextureType.Toon);
                }
            }
