...
public partial class PmxRenderer
{
    ...
    private void EnsureBoneCapacity()
    {
        ...
        _boneMatrices = new float[Math.Min(_boneCapacity, MaxBones) * 16];
    ...
    private void CpuSkinning()
    {
        ...
        int uploadCount = Math.Min(_bones.Count, MaxBones);
        if (_boneMatrices.Length != uploadCount * 16)
            _boneMatrices = new float[uploadCount * 16];
        for (int i = 0; i < uploadCount; i++)
        {
            var m = _skinMats[i];
            int o = i * 16;
            _boneMatrices[o + 0] = m.M11; _boneMatrices[o + 1] = m.M12; _boneMatrices[o + 2] = m.M13; _boneMatrices[o + 3] = m.M14;
            _boneMatrices[o + 4] = m.M21; _boneMatrices[o + 5] = m.M22; _boneMatrices[o + 6] = m.M23; _boneMatrices[o + 7] = m.M24;
            _boneMatrices[o + 8] = m.M31; _boneMatrices[o + 9] = m.M32; _boneMatrices[o +10] = m.M33; _boneMatrices[o +11] = m.M34;
            _boneMatrices[o +12] = m.M41; _boneMatrices[o +13] = m.M42; _boneMatrices[o +14] = m.M43; _boneMatrices[o +15] = m.M44;
        }
    ...
    {
        bool needsUpdate = _morphDirty || _uvMorphDirty;
        if (!needsUpdate)
        {
            _bonesDirty = false;
            return;
    ...
    private void DrawScene()
    {
        ...
        #endif
        if (_bones.Count > 0 && _boneMatrices.Length >= Math.Min(_bones.Count, MaxBones) * 16)
        {
            int count = Math.Min(_bones.Count, MaxBones);
            GL.UniformMatrix4(_modelBonesLoc, count, false, _boneMatrices);
            #if DEBUG
            if (!CheckGLError("GL.UniformMatrix4", $"loc={_modelBonesLoc}, count={count}")) return;
            #endif
        }
        RenderMesh[] meshes;
        lock (_meshesLock)
