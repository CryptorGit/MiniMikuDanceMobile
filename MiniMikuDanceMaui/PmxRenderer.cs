...
void main(){
    ...
    if(uUseSphereTex){
        ...
        if(uSphereMode == 1) base = mix(base, base * s, uSphereStrength);
        else if(uSphereMode == 2){
            base.rgb = clamp(base.rgb + s.rgb * uSphereStrength, 0.0, 1.0);
            base.a   = clamp(base.a + s.a * uSphereStrength, 0.0, 1.0);
        }
    }
    if(uUseToonTex){
        vec3 t = texture(uToonTex, vTex).rgb;
        base.rgb = mix(base.rgb, base.rgb * t, uToonStrength);
    }
