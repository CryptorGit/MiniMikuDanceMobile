...
void main(){
    ...
    private static (int Texture, bool HasTexture) LoadTextureBytes(byte[]? bytes, int width, int height)
    {
        ...
        try
        {
            GL.TexImage2D((All)TextureTarget.Texture2D, 0, (All)PixelInternalFormat.Rgba, width, height, 0, (All)PixelFormat.Rgba, (All)PixelType.UnsignedByte, handle.AddrOfPinnedObject());
